...
def index(request):
    ...
    new_suggestions_list = [x for x in list(all_users) if x not in list(chain(*user_following_all))]
...
def search(request):
    ...
    if request.method == 'POST':
        ...
        for user in username_object:
            username_profile.append(user.id)
        for userid in username_profile:
            profile_list = Profile.objects.filter(id_user=userid)
